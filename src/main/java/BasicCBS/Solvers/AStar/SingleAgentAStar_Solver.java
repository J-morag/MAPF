...
//        this.closed = new HashSet<>();
    ...
    protected boolean initOpen() {
        ...
        if(existingPlan.size() > 0){
            ...
            openList.add(newState(existingPlan.moveAt(existingPlan.getEndTime()), null,
    ...
    protected AStarState newState(Move move, AStarState prevState, int g, int conflicts) {
        return new AStarState(move, prevState, g, conflicts);
    }
    ...
    protected void fillOpenWithRoots() {
        ...
        for (I_Location destination: neighborCellsIncludingCurrent) {
            ...
            if (constraints.accepts(possibleMove)) { //move not prohibited by existing constraint
                AStarState rootState = newState(possibleMove, null, 1, 0);
    ...
    public class AStarState implements Comparable<AStarState>{
        ...
        public void expand() {
            ...
            for (I_Location destination: neighborCellsIncludingCurrent){
                ...
                if(constraints.accepts(possibleMove)){ //move not prohibited by existing constraint
                    AStarState child = newState(possibleMove, this, this.g + 1, conflicts + numConflicts(possibleMove));
