...
public class DistanceTableSingleAgentHeuristic implements SingleAgentGAndH {
     ...
     */
    public DistanceTableSingleAgentHeuristic(@Nullable List<? extends Agent> agents, I_Map map, int lruCacheSize) {
        this.map = map;
     ...
     * @param lruCacheSize the size of the LRU cache used to store the distance dictionaries. If <= 0, no cache is used.
     */
    public DistanceTableSingleAgentHeuristic(@NotNull I_ExplicitMap map, int lruCacheSize) {
        this.map = map;
