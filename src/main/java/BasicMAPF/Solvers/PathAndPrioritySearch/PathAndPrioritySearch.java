...
public class PathAndPrioritySearch extends A_Solver {
    ...
    private final I_PaPSHeuristic highLevelHeuristic;
    ...
    PathAndPrioritySearch(@Nullable I_OpenList<PaPSNode> openList, @Nullable Comparator<PaPSNode> nodeComparator,
                          ...
                          @Nullable Integer MDDCacheDepthDeltaMax, @Nullable I_PaPSHeuristic highLevelHeuristic,
        ...
        this.highLevelHeuristic = Objects.requireNonNullElseGet(highLevelHeuristic, PaPSHeuristicSIPP::new);
        ...
        super.name = "Path and Priority Search";
    }
    ...
    void addToOpen(@NotNull PaPSNode node) {
        if (useDuplicateDetection && closedList.contains(node)){
            if (INFO >= 2){
                System.out.println(this.getClass().getSimpleName() + " - Closed duplicate node skipped: " + node.hashCode() + " open list size " + openList.size() + " closed list size " + closedList.size());
            }
        ...
        if (useDuplicateDetection && openList.contains(node)){
            if (INFO >= 2){
                System.out.println(this.getClass().getSimpleName() + " - Open duplicate node skipped: " + node.hashCode() + " open list size " + openList.size() + " closed list size " + closedList.size());
            }
...
//        List<Constraint> addedConstraints = new ArrayList<>(criticalResourceConstraints); //for now, misses the already added constraints because we don't save them and don't use them for anything
    ...
    protected int[] getHarr(ArrayList<MDD> MDDs, ConstraintSet updatedConstraints, Agent[] priorityOrderedAgents) {
        ...
        if (highLevelHeuristic instanceof PaPSHeuristicSIPP){
            safeIntervalsByLocation = updatedConstraints.vertexConstraintsToSortedSafeTimeIntervals(null, this.currentMap);
        }
        return highLevelHeuristic.getH(priorityOrderedAgents, MDDs.size(), updatedConstraints, currentInstance, singleAgentHeuristic, safeIntervalsByLocation);
    }
