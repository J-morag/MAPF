...
public class PCSHeuristicSIPP implements I_PCSHeuristic {
    ...
    @Override
    public int[] getH(Agent[] priorityOrderedAgents, int numMDDsAlreadyInNode, @NotNull I_ConstraintSet constraints,
...
//            long timeLeftToTimeout = Math.max(super.maximumRuntime - (Timeout.getCurrentTimeMS_NSAccuracy() - super.startTime), 0);
            // todo timeout?
            int shortestPathLength = singleAgentSolver.solve(currentInstance.getSubproblemFor(agent),
                    new RunParametersBuilder().setAStarGAndH(singleAgentHeuristic).setInstanceReport(new InstanceReport())
                            .setConstraints(constraints).createRP()).getPlanFor(agent).getCost();
